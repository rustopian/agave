...
pub fn parse_system(
    ...
    match system_instruction {
        ...
        SystemInstruction::CreatePrefundedAccount {
            ...
            check_num_system_accounts(&instruction.accounts, 2)?;
            Ok(ParsedInstructionEnum {
                instruction_type: "createAccountUnchecked".to_string(),
                info: json!({
                    "source": account_keys[instruction.accounts[0] as usize].to_string(),
                    "newAccount": account_keys[instruction.accounts[1] as usize].to_string(),
                    "lamports": lamports,
...
mod test {
    ...
    #[test]
    fn test_parse_system_create_prefunded_account_ix() {
        let lamports = 55;
        let space = 128;
        let from_pubkey = Pubkey::new_unique();
        let to_pubkey = Pubkey::new_unique();
        ...
        let instruction = system_instruction::create_prefunded_account(
            &from_pubkey,
            &to_pubkey,
            lamports,
            space,
